...
Requirements: A running instance of Redis, a MongoDB cluster and Firebase Storage credentials.
...
|---------------|---------|----------|------|:----:|--------|
|Register a new user|POST|`/auth/register`|\<userSchema\>|-|        |
|Login a registered user|POST|`/auth/login`|\<userSchema\>|-|        |
|Generate a new access token|POST|`/auth/refresh-token`|\<refreshToken\>|-|        |
...
|Upload an Image|POST|`/file/new`|\<userId\>|Bearer \<accessToken\>|        |
|Get an Image|GET|`/file/{imageId}`|\<userId\>|Bearer \<accessToken\>|        |
|Delete an Image|DELETE|`/file/{imageId}`|\<userId\>|Bearer \<accessToken\>|        |
|Create a post|POST|`/post/new`|\<postSchema\>|Bearer \<accessToken\>|        |
...
|Get number of likes|GET|`/post/likes/{postId}`|\<userId\>|Bearer \<accessToken\>|        |
|Get number of following likes|GET|`/post/flikes/{postId}`|\<userId\>|Bearer \<accessToken\>|        |
|Toggle save on a post|PUT|`/post/save/{postId}`|\<userId\>|Bearer \<accessToken\>|        |
|Render a paginated feed|GET|`/feed/{pageIndex}`|\<userId\>|Bearer \<accessToken\>|        |
